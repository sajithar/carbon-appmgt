...
public class AppMDAO {
	...
	public APIInfoDTO[] getSubscribedAPIsOfUser(String userId) throws AppManagementException,
		...
		ResultSet rs = null;
		String sqlQuery =
		                  "SELECT " + "   API.APP_PROVIDER AS APP_PROVIDER,"
		                          + "   API.APP_NAME AS APP_NAME,"
		                          + "   API.APP_VERSION AS APP_VERSION " + "FROM "
		                          + "   APM_SUBSCRIPTION SP, " + "   APM_APP API,"
		                          + "   APM_SUBSCRIBER SB, " + "   APM_APPLICATION APP " + "WHERE "
		                          + "   SB.USER_ID = ? " + "   AND SB.TENANT_ID = ? "
	...
	public APIKeyInfoDTO[] getSubscribedUsersForAPI(APIInfoDTO apiInfoDTO)
		...
		ResultSet rs = null;
		String sqlQuery =
		                  "SELECT " + "   SB.USER_ID, " + "   SB.TENANT_ID " + "FROM "
		                          + "   APM_SUBSCRIBER SB, " + "   APM_APPLICATION APP, "
		                          + "   APM_SUBSCRIPTION SP, " + "   APM_APP API " + "WHERE "
		...
		try {
			...
			while (rs.next()) {
				...
				// int tenantId =
				// rs.getInt(AppMConstants.SUBSCRIBER_FIELD_TENANT_ID);
				// If the tenant Id > 0, get the tenant domain and append it to
				// the username.
				// if (tenantId > 0) {
				// userId = userId + "@" +
				// APIKeyMgtUtil.getTenantDomainFromTenantId(tenantId);
				// }
	...
	public void changeAccessTokenStatus(String userId, APIInfoDTO apiInfoDTO, String statusEnum)
		...
		PreparedStatement ps = null;
		String sqlQuery =
		                  "UPDATE " + accessTokenStoreTable + " IAT , APM_SUBSCRIBER SB," +
		                          " APM_SUBSCRIPTION SP , APM_APPLICATION APP, APM_APP API" +
		                          " SET IAT.TOKEN_STATE=?" + " WHERE SB.USER_ID=?" +
		                          " AND SB.TENANT_ID=?" + " AND API.APP_PROVIDER=?" +
	...
	public APIKeyValidationInfoDTO validateKey(String context, String version, String accessToken,
		...
		String applicationSqlQuery =
		                             "SELECT " + "   IAT.VALIDITY_PERIOD, " +
		                                     "   IAT.TIME_CREATED ," + "   IAT.TOKEN_STATE," +
		                                     "   IAT.USER_TYPE," + "   IAT.AUTHZ_USER," +
		                                     "   IAT.TIME_CREATED," + "   SUB.TIER_ID," +
		                                     "   SUBS.USER_ID," + "   SUB.SUB_STATUS," +
    ...
    public ArrayList<String> getInUrlMappingById(int appId)
        ...
        String ssoInfoSqlQuery = "SELECT URL_PATTERN FROM APM_APP_URL_MAPPING WHERE " +
                "APP_ID= ?";
            try {
                conn = APIMgtDBUtil.getConnection();
	...
	public long getApplicationAccessTokenRemainingValidityPeriod(String accessToken)
		...
		String applicationSqlQuery =
		                             "SELECT " + " IAT.VALIDITY_PERIOD, " + " IAT.TIME_CREATED " +
		                                     " FROM " + accessTokenStoreTable + " IAT" + " WHERE " +
		                                     " IAT.ACCESS_TOKEN = ? ";
...
public Set<Subscriber> getSubscribersOfAPI(APIIdentifier identifier)
	...
	public void addWebApp(WebApp app) throws AppManagementException {
		...
		try {
			...
			String gatewayURLs = ServiceReferenceHolder.getInstance().getAPIManagerConfigurationService().getAPIManagerConfiguration().getFirstProperty(GATEWAY_URL);
	...
	public WorkflowDTO retrieveWorkflow(String workflowReference) throws AppManagementException {
		...
		String query = "SELECT * FROM APM_WORKFLOWS WHERE WF_EXTERNAL_REFERENCE=?";
		try {
	...
	public void addURLTemplates(int apiId, WebApp api, Connection connection) throws
		...
		String query =
				"INSERT INTO APM_APP_URL_MAPPING (APP_ID,HTTP_METHOD,URL_PATTERN,POLICY_GRP_ID) VALUES (?,?,?,?)";

		if (log.isDebugEnabled()) {
			log.debug("Inserting Application's URL Mappings and Entitlement policy partial mappings");
		}
	...
	public static ArrayList<URITemplate> getAllURITemplates(String apiContext, String version)
		...
		String query =
				" SELECT AUM.HTTP_METHOD,AUTH_SCHEME,URL_PATTERN,THROTTLING_TIER,SKIP_THROTTLING,USER_ROLES "
		...
		try {
			...
			if (connection != null) {
				try {
					...
					log.error("Failed to rollback the updating the WebApp" + api.getId() + " in the database", e);
				}
			}
			handleException("Error while updating the WebApp: " + api.getId() + " in the database",
					e);
		} finally {
			APIMgtDBUtil.closeAllConnections(prepStmt, connection, rs);
		}
	...
	public Comment[] getComments(APIIdentifier identifier) throws AppManagementException {
		...
		String sqlQuery =
				"SELECT " + "   APM_APP_COMMENTS.COMMENT_TEXT AS COMMENT_TEXT,"
   ...
   /**
	...
	public List<APIIdentifier> getAssociatedApps(int policyPartialId) throws AppManagementException {
		...
		try {
			...
			handleException("Failed to retrieve apps associated with policy partial ->" +
					"policy partial id:" + policyPartialId, e);
		} finally {
	...
	private void removeApplicationsEntitlementPolicies(int applicationId, Connection connection) throws
		...
		try {
			...
			while (rs.next()) {
				...
				//If policyid not null, remove the Entitlement policy with reference to policy id
				if (policyId != null) {
	...
	public APIKeyValidationInfoDTO getApplicationData(String appContext, String appVersion,
													  String consumer, AuthenticatedIDP[] authenticatedIDPs) throws
			AppManagementException {
		...
		try {
			...
			if(!hasValidSubscription){
				info.setValidationStatus(900908);
	 ...
	 * @throws AppManagementException on error
	 */
	public JSONArray getEntitledPartialListForPolicyGroup(Integer policyGroupId, Connection connection) throws
			AppManagementException {
		PreparedStatement ps = null;
		ResultSet rs = null;
